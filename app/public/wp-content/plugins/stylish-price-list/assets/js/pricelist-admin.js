function add_service( service_link ) {
	...
	const service_rows = category_row.find( '.service-price-length-rows-wrapper' );
	...
	if ( evaluation ) {
		...
		const service_rows_clone = jQuery( '#category-row-template .service-price-length-rows-wrapper' ).clone();
		const service_rows_wrapper = get_service_rows_from_add_remove_service_link( service_link );
...
function remove_service( service_link ) {
	service_link = jQuery( service_link ); const category_row = get_category_row_from_add_remove_service_link( service_link ); const service_row = get_service_rows_from_add_remove_service_link( service_link ); service_row.remove(); update_all_service_rows_html_in_wrapper( category_row ); const service_rows = category_row.find( '.service-price-length-rows-wrapper' ); if ( 0 == service_rows.length ) {
		category_row.remove();
	}
}
