...
class SPL_LicensePage {
	...
	public function page() {
		...
		if ( isset( $_POST['submit'] ) ) {
			...
			switch ( $_POST['_action'] ) {
				case 'active':
					$this->active_client_license( $_POST );
					break;

				case 'deactivate':
					$this->deactive_client_license( $_POST );
					break;

				case 'refresh':
					$this->refresh_client_license( $_POST );
					break;
			}
		}
		...
		<div id="license-page-wrapper" class="container-fluid my-5">
			...
			if ( ! empty( $yy->error ) ) {
				?>
				<div class="notice notice-error is-dismissible appsero-license-section m-0">
					<p><?php echo wp_kses_post( $yy->error ); ?></p>
				</div>
				<?php
			}
			...
			if ( ! empty( $yy->success ) ) {
				?>
					<div class="notice notice-success is-dismissible appsero-license-section m-0">
						<p><?php echo wp_kses_post( $yy->success ); ?></p>
					</div>
				<?php
			}
				echo '<br />';
					...
					<div class="p-5 bg-white">
						<div class="text-center">
							...
							<div class="input-group flex-nowrap mb-3 license-input-wrapper">
								...
								<input type="text" class="form-control"  value="<?php echo esc_attr($stylishpl_license_key); ?>"
									placeholder="<?php echo esc_attr( ( 'Enter your license key to activate' ) ); ?>" name="license_key"
									<?php echo ( 'deactivate' == $action ) ? 'readonly="readonly"' : ''; ?>>
								<form method="post" class="appsero-license-right-form" action="<?php // $yy->get_action_url(); ?>" novalidate="novalidate" spellcheck="false">
						...
						<?php if ( $license['status'] !== 'deactivate' ) : ?>
							...
							<div class="activation-text">
								...
								<?php if ( ! empty( $opt ) && in_array( $opt['license'] , array( 'valid', 'expired' ) ) ): ?>
									...
									<?php endif; ?>
									<?php if ( isset( $opt['expires'] ) && $opt['expires'] != 'lifetime' ) : ?>
									...
									<?php if ( isset( $opt['license_limit'] ) && isset( $opt['site_count'] ) ): ?>
										<p class="mb-0">License key is being used on <?php echo ( intval( $opt['site_count'] ) > 1 ) ? intval( $opt['site_count'] ) . ' sites' : intval( $opt['site_count'] ) . ' site'; ?> out of <?php echo intval( $opt['license_limit'] ); ?> allowed.</p>
									<?php endif; ?>
								<?php endif; ?>
							</div>
		...
		?>
			...
			<div class="col-md-4">
				<div class="px-3 py-5 bg-white">
					...
					<ul class="p-0">
						<li class="mb-4"><a href="https://designful.freshdesk.com/a/solutions/articles/48001146692" target="_blank">Click here to find out how to migrate/transfer your license keys.</a></li>
						<li><a href="https://designful.freshdesk.com/a/solutions/articles/48001190711" target="_blank">Member's Portal (Transfer Licenses / Cancel Subscription</a></li>
					</ul>
				</div>
			</div>
			<?php endif; ?>
			...
			<div class="col-md-4">
				<div class="px-3 py-5 bg-white">
					...
					<ul class="p-0">
						<li><a href="#">Manage Existing Licenses</a></li>
						<li><a href="#">Access Member's Portal</a></li>
					</ul>
				</div>
			</div>
			<?php endif; ?>
	...
	private static function page_script() {
		...
		<script>
			const handleLicenseBtn = ($this, evt) => {
				...
				formObj.append('license_key', document.querySelector('[name="license_key"]').value);
				formObj.append('_action', action);
				formObj.append('submit', 'submit');
				formObj.append('_nonce', document.querySelector('[name="_nonce"]').value);

				jQuery.ajax({
					type: 'POST',
					url: "<?php echo self::get_action_url(); ?>",
					data: formObj,
					srcBtn: $this,
					processData: false,
					contentType: false,
					beforeSend: (evt, {srcBtn}) => {
						srcBtn.querySelector('.gg-spinner-alt').classList.remove('d-none');
					}
				}).done(function (data) {
	...
	// 	$route = 'public/license/' . 'habijabi' . '/activate';
	...
	private function refresh_client_license( $license_key ) {
		...
		// SCC_Update_Services::update_check();
	}
