...
if ( ! empty( $id ) ) {
	...
	foreach ( $opt_cats as $cat_id => $cat ) {
		...
		foreach ( $cat as $service_id => $service ) {
			$services[ $service_id ]['name']                     = df_spl_remove_slash_quotes( $service['service_name'] );
			$services[ $service_id ]['regular_price']            = df_spl_remove_slash_quotes( isset( $service['service_regular_price'] ) ? $service['service_regular_price'] : '' );
			$services[ $service_id ]['price']                    = df_spl_remove_slash_quotes( $service['service_price'] );
			$services[ $service_id ]['desc']                     = df_spl_remove_slash_quotes( $service['service_desc'] );
			$services[ $service_id ]['service_button']           = df_spl_remove_slash_quotes( isset( $service['service_button'] ) ? $service['service_button'] : '' );
			$services[ $service_id ]['service_button_url']       = df_spl_remove_slash_quotes( isset( $service['service_button_url'] ) ? $service['service_button_url'] : '' );
			$services[ $service_id ]['service_url']              = df_spl_remove_slash_quotes( isset( $service['service_url'] ) ? $service['service_url'] : '' ); //remove_slash_quotes($service['service_url']);
			$services[ $service_id ]['service_image']            = df_spl_remove_slash_quotes( isset( $service['service_image'] ) ? $service['service_image'] : '' ); //remove_slash_quotes($service['service_image']);
			$services[ $service_id ]['service_long_description'] = df_spl_remove_slash_quotes( isset( $service['service_long_description'] ) ? $service['service_long_description'] : '' );
		}
	...
	function output_service_style6_two_column( $service, $is_buy_btn_newtab_enabled ) {
		...
		<div class="style-6-two-column">
			...
			<div class="style-6-section spl-position-relative">
				...
				<h4 class="style-6-2-desc"><strong class="name a-tag"><?php echo html_entity_decode( $service['name'] ); ?></strong></h4>
				<h4 class="style-6-2-price"><strong class="spl-price a-tag"><?php echo html_entity_decode( $service['price'] ); ?></strong></h4>
			</div>
			<div class="style-6-height-10"></div>
			<div class="style-6-2-section name-price-desc">
				<p class="st-6-fl-l desc a-tag"><?php echo html_entity_decode($service['desc']); ?></p>
				<?php
...
if ( ! function_exists( 'output_service' ) ) {
	function output_service( $service, $is_buy_btn_newtab_enabled ) {
		...
		ob_start();
		?>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 name-price-desc" style="
		<?php
		if ( ! empty( $service['service_image'] ) ) {
			echo esc_attr( 'display: flex;' );}
		?>
					...
					<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3" style="padding:0;">
						<div class="df-spl-level">
							<div class="df-spl-level-left" style="display: block">
								<?php echo df_spl_output_a_tag( $price, '', 'spl-price a-tag' ); ?>
								<a href="<?php echo esc_attr($service['service_button_url']); ?>" <?php echo isset( $newTabOpen ) ? esc_attr($newTabOpen) : ''; ?> class="btn btn-book-now spl_book_now_btn_style_3"><?php echo esc_attr($service['service_button']); ?></a>
							</div>
						</div>
					</div>
				<?php } else { ?>
...
if ( ! function_exists( 'output_service_style4_col1' ) ) {
	function output_service_style4_col1( $service, $is_buy_btn_newtab_enabled ) {
		...
		ob_start();
		?>
		<div class="internal-box clearfix" style="padding:0px">
			<?php if ( ! empty( $service['service_button'] ) ) { ?>
				...
				if ( $cat['description'] != '' ) {
					...
					<div class="df-spl-row">
						<div class="col-sm-12 custom-description-section">
							<?php echo html_entity_decode( nl2br( $cat['description'] ) ); ?>
						</div>
					</div>
				<?php } ?>
				<div class="df-spl-row">
...
if ( ! function_exists( 'output_tabs_style5' ) ) {
	function output_tabs_style5( $cats, $default, $shortcode_id, $is_buy_btn_newtab_enabled ) {
		...
		<?php foreach ( $cats as $key => $cat ) : ?>
			...
			?>
						">
				<a href="javascript:void(0)" data-href="#<?php echo esc_attr($key) . '_' . $shortcode_id; ?>"><?php echo html_entity_decode( $name ); ?></a>
			</li>
		<?php endforeach ?>
		<?php
		$html = ob_get_clean();
...
if ( ! function_exists( 'output_tab_contents_style8' ) ) {
	function output_tab_contents_style8( $cats, $default, $shortcode_id, $select_column, $is_buy_btn_newtab_enabled ) {
		ob_start();
		?>
		<?php
		$all_services = array();
...
if ( ! function_exists( 'df_spl_output_a_tag' ) ) {
	function df_spl_output_a_tag( $text, $id = '', $class = '' ) {
		ob_start();
		?>
		<div class="<?php echo esc_attr($class); ?>"><?php echo html_entity_decode($text); ?></div>
		<?php
		$html = ob_get_clean();
		return $html;
		...
		<div class="tab <?php echo ( $default ) ? '' : 'active'; ?>" id="all_<?php echo esc_attr($shortcode_id); ?>" style="<?php echo ( $default ) ? 'display:none' : 'display:block'; ?>">
			<div class="df-spl-row">
				<div class="<?php echo esc_attr( $columns ); ?>">
					...
					<?php
					foreach ( $all_services as $key => $service ) {
						echo array_key_exists( $i, $all_services ) ? output_service_style6_two_column( $all_services[ $i ], $is_buy_btn_newtab_enabled ) : false;
						$i = $i + 2;
					}
					?>
				</div>
				<div class="<?php echo esc_attr( $columns ); ?>">
					...
					<?php
					foreach ( $all_services as $key => $service ) {
						echo array_key_exists( $j, $all_services ) ? output_service_style6_two_column( $all_services[ $j ], $is_buy_btn_newtab_enabled ) : false;
						$j = $j + 2;
					}
					?>
				</div>
			...
			<div class="tab <?php echo esc_attr($act_tab); ?>" id="<?php echo esc_attr($key) . '_' . $shortcode_id; ?>" style="<?php echo esc_attr($style_act); ?>">
				...
				<div class="df-spl-row">
					<div class="<?php echo esc_attr( $columns ); ?>">
						...
						<?php
						foreach ( $services as $key => $service ) {
							echo array_key_exists( $i, $services ) ? output_service_style6_two_column( $services[ $i ], $is_buy_btn_newtab_enabled ) : false;
							$i = $i + 2;
						}
						?>
					</div>
					<div class="<?php echo esc_attr( $columns ); ?>">
						...
						<?php
						foreach ( $services as $key => $service ) {
							echo array_key_exists( $j, $services ) ? output_service_style6_two_column( $services[ $j ], $is_buy_btn_newtab_enabled ) : false;
							$j = $j + 2;
						}
						?>
					</div>
...
if ( ! function_exists( 'output_service_style8' ) ) {
	function output_service_style8( $service, $is_buy_btn_newtab_enabled ) {
		...
		ob_start();
		?>
		<div class="style-8-card clearfix">
		...
		foreach ( $cats as $key => $cat ) {
			...
			<?php if ( $cat['description'] ) { ?>
				<div class="cat_descreption row clearfix" style="padding:0;">
					<div class="col-sm-12" style="padding:0;"><?php echo html_entity_decode( $cat['description'] ); ?></div>
				</div>
			<?php } ?>
		...
		foreach ( $cats as $key => $cat ) {
			...
			<?php if ( $cat['description'] ) { ?>
				<div class="cat_descreption row" style="padding:0;">
					<div class="col-sm-12" syle="padding:0"><?php echo html_entity_decode( $cat['description'] ); ?></div>
				</div>
			<?php } ?>
							...
							<div class="name-price-desc for-style-3">
								...
								<?php if ( $cat['description'] ) { ?>
									<div class="cat_descreption row" style="padding-top:5px;">
										<div class="style3_cat_desc" style="padding:0"><?php echo html_entity_decode( $cat['description'] ); ?></div>
									</div>
								<?php } ?>
								<?php
								foreach ( $services as $key => $service ) {
	...
	function output_tab_contents_fourth_style_col1( $cats, $default, $shortcode_id, $is_buy_btn_newtab_enabled ) {
		...
		<ul id="tiles">
			...
			foreach ( $cats as $key => $cat ) {
				...
				<!-- These are our grid blocks -->
				<li>
					<div class="name-price-desc for-style-4 style-4-half"><span class="head-title tab-links_spl head_title_style_3 spl_cat_heading_style_4"><?php echo html_entity_decode( $cat['name'] ); ?></span>
																																									   <?php
																																										if ( $cat['description'] ) {
																																											?>
						<div class="cat_descreption row">
								<div class="style4_cat_desc"><?php echo html_entity_decode( $cat['description'] ); ?></div>
							</div>
							<?php } ?>
									<?php
									foreach ( $services as $key => $service ) {
...
if ( ! function_exists( 'output_tab_contents_4_style_break_col1' ) ) {
	function output_tab_contents_4_style_break_col1( $cats, $default, $shortcode_id, $is_buy_btn_newtab_enabled ) {
		foreach ( $cats as $key => $cat ) {
			...
			<?php if ( $cat['description'] ) { ?>
				<div class="cat_descreption row">
					<div class="col-sm-6"><?php echo html_entity_decode( $cat['description'] ); ?></div>
				</div>
			<?php } ?>
			<div class="df-spl-row">
				<?php
...
if ( $style == 'style_8' ) :
	...
	<div class="body-inner container-fluid price_wrapper spl_main_content_box" id="spl_<?php echo esc_attr($id); ?>" data-config=<?php echo esc_js( json_encode( $pricelist_config ) ); ?> data-style="<?php echo esc_attr($style); ?>" style="max-width:1200px;margin-left:auto;margin-right:auto;">
		<div class="style-8-container">
			...
			<?php endif; ?>
			<?php echo output_tab_contents_style8( $cats, $default, $id, $select_column, $is_buy_btn_newtab_enabled ); ?>
		</div>
	</div>
<?php endif; ?>
...
<?php if ( $style == 'style_10' ) :
	...
	<div class="style-10 spl_main_content_box" id="spl_<?php echo esc_attr($id); ?>">
		...
		<?php foreach ($cats as $cat_key => $current_cat) {
			...
			<div class="spl-s10-cat-wrapper" id="<?php echo esc_attr( $cat_key.'_'.$id ); ?>">
				...
				<ul class="spl-s10-list-wrapper">
					<?php foreach ($current_cats_items as $cat_item_key => $item_data) { ?>	
						<li class="spl-s10-list-item">
							<div class="item-title-desc-wrapper">
								<p class="spl-s10-bold"><?php echo sanitize_text_field($item_data['name']); ?></p>
							...
							<?php if (! ( empty( $item_data['price'] ) && empty( $item_data['service_button_url'] ) && empty( $item_data['service_button'] ) ) ) : ?>
								<div class="item-btn-wrapper">
									<div>
										<p><?php echo sanitize_text_field($item_data['price']); ?></p>
	...
	$footer_content = function () use($id, $cats) {
		...
		<div class="spl-style-10-footer-bottom" id="spl10_<?php echo esc_attr($id); ?>">
			<div class="spl-style-10-footer-wrapper">
				<?php foreach ($cats as $cat_key => $current_cat) {
					?>
					<a href="<?php echo esc_attr( '#'.$cat_key.'_'.$id ); ?>" class="spl-style-10-nav"><?php echo esc_attr( $current_cat['name'] ); ?></a>
					<?php
				} ?>
				<!-- <a id="FID10548" href="#MC10548" data-filter=".MC10548" class="menu-category small text-muted text-uppercase p-1 px-2 mr-2 rounded-lg border shadow-xs">COLD STARTERS</a>
				<a id="FID10549" href="#MC10549" data-filter=".MC10549" class="menu-category small text-muted text-uppercase p-1 px-2 mr-2 rounded-lg border shadow-xs active">HOT STARTERS</a>
				<a id="FID10550" href="#MC10550" data-filter=".MC10550" class="menu-category small text-muted text-uppercase p-1 px-2 mr-2 rounded-lg border shadow-xs">MAIN DISHES</a><a id="FID10551" href="#MC10551" data-filter=".MC10551" class="menu-category small text-muted text-uppercase p-1 px-2 mr-2 rounded-lg border shadow-xs">FITTINGS</a><a id="FID10552" href="#MC10552" data-filter=".MC10552" class="menu-category small text-muted text-uppercase p-1 px-2 mr-2 rounded-lg border shadow-xs">DESSERTS</a> -->
			</div>
		</div>
		<?php
	};
...
<?php if ($style == 'style_10'): ?>
	<style type="text/css">
		...
		#spl10_<?php echo esc_attr($id) ?> {
			...
			/* --spl-s10-cat-desc-font-size: inherit;
			--spl-s10-cat-desc-color: #fff;
			--spl-s10-cat-desc-font-style: Gothic A1;
			--spl-s10-cat-desc-font-weight: 500;

			--spl-s10-category-font-size: 30px;
			--spl-s10-category-color: #65b5a8;
			--spl-s10-category-font-style: Playfair Display;
			--spl-s10-category-font-weight: 600;
			
			--spl-s10-item-name-color: #000000;
			--spl-s10-item-name-font-style: Gothic A1;
			--spl-s10-item-name-hover-color: #000000;
			--spl-s10-item-name-font-weight: 600; */
		}
		.spl-s10-list-wrapper {
			margin-left: 0;
		}
		.spl-s10-cat-manifest .spl-s10-cat-name {
			margin-top: 0;
			margin-bottom: 12px;
			color: var(--spl-s10-category-color);
			font-family: var(--spl-s10-category-font-style);
			font-size: var(--spl-s10-category-font-size);
			font-weight: var(--spl-s10-category-font-weight);
		}
		.spl-s10-cat-desc {
			color: var(--spl-s10-cat-desc-color);
			font-family: var(--spl-s10-cat-desc-font-style);
			font-size: var(--spl-s10-cat-desc-font-size);
			font-weight: var(--spl-s10-cat-desc-font-weight);
		}
		.spl-s10-cat-wrapper {
			box-shadow: 0 3px 6px 0 rgb(0 0 0 / 10%), 0 1px 3px 0 rgb(0 0 0 / 8%);
			border-radius: 0.3rem;
		}
		.spl-s10-cat-wrapper:not(:last-child) {
			margin-bottom: 25px;
		}
		.spl-s10-cat-cover-image {
			height: 250px;
			font-size: 160%;
		...
		.spl-s10-list-item .item-title-desc-wrapper + .item-btn-wrapper {
			width: 20%;
		}
		.spl-s10-list-item .item-btn-wrapper {
			text-align: end;
		}
		.spl-s10-list-item:not(:last-child) {
			border: 1px solid rgba(0,0,0,.125);
			border-right-width: 0;
			border-left-width: 0;
			border-radius: 0;
		...
		.spl-s10-bold {
			font-weight: var(--spl-s10-item-name-font-weight);
			font-family: var(--spl-s10-item-name-font-style);
			font-size: var(--spl-s10-item-name-font-size);
			color: var(--spl-s10-item-name-color);
		}
		.spl-s10-bold:hover {
			color: var(--spl-s10-item-name-hover-color);
		}

		.spl-s10-title {
			padding: 1rem 0;
			text-align: center;
			color: var(--spl-s10-title-color);
			font-size: var(--spl-s10-title-font-size);
			font-family: var(--spl-s10-title-font-style);
			font-weight: var(--spl-s10-title-font-weight);
		}
		.spl-s10-muted {
			color: var(--spl-s10-desc-color);
			font-size: var(--spl-s10-desc-font-size);
			font-family: var(--spl-s10-desc-font-style);
			font-weight: var(--spl-s10-desc-font-weight);
		}
		.spl-s10-list-item .item-btn-wrapper p {
			color: var(--spl-s10-price-color);
			font-size: var(--spl-s10-price-font-size);
			font-family: var(--spl-s10-price-font-style);
			font-weight: var(--spl-s10-price-font-weight);
		}
		.spl-s10-list-item .item-btn-wrapper a {
			background-color: var(--spl-s10-price-color);
			color: #fff;
			padding: 5px 8px;
			border-radius: 5px;
		}
		.spl-style-10-footer-bottom {
			width: 100%;
			height: auto;
			position: fixed;
			bottom: 0;
		...
		.spl-style-10-footer-wrapper a.active {
			box-shadow: 0 0.1rem 0.5rem #ffffff90 !important;
			font-size: 15px;
		}
		.spl-style-10-footer-wrapper {
			justify-content: center;
			background-color: var(--spl-s10-category-color);
		}
	</style>
<?php endif; ?>
<!--AK Style -->
<style type="text/css">
  ...
  font-weight: 400;
	...
	#spl_<?php echo esc_attr($id); ?>.style-4-border {
		background-color: <?php echo esc_attr($service_description_color); ?>;
	}
	...
	#spl_<?php echo esc_attr($id); ?>.price_wrapper span.tab-links_spl:hover {
		color: <?php echo esc_attr($hover_color); ?> !important;
	}
	...
	#spl_<?php echo esc_attr($id); ?>.price_wrapper .spl_book_now_btn_style_3:hover {
		background-color: <?php echo esc_attr($title_color_top); ?> !important;
		color: #fff;
		font-family: <?php echo splPrintFontName( $title_font ); ?>;
	}
	#spl_<?php echo esc_attr($id); ?>.price_wrapper .spl_book_now_btn_style_3 {
		font-family: <?php echo splPrintFontName( $title_font ); ?>;
	}

	/*Book now button style 3*/
	#spl_<?php echo esc_attr($id); ?>.spl_book_now_btn_style_3,
